...
def correlation(img, mask, threshold):
    ...
    new_img = np.zeros((img.shape[0], img.shape[1]))
    eximg = cv2.copyMakeBorder(img, half_mask_size, half_mask_size, half_mask_size, half_mask_size, cv2.BORDER_REPLICATE)
...
if __name__ == '__main__':
    ...
    m3 = [[2 / 3, -1 / 3, 2 / 3],
          [-1 / 3, -4 / 3, -1 / 3],
          [2 / 3, -1 / 3, 2 / 3]]
    min_laplacian = zero_crossing(correlation(img, m3, 15))
